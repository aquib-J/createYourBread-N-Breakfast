...
class PaymentService {
  ...
  static async verifyPaymentSignature(params) {
      try {
          ...
          const hashedSignature = crypto.createHmac('sha256', razorpayConfig.keySecret).update(signature.toString()).digest('hex');

          if (params.razorpay_signature === hashedSignature) {
              return { status: "success" };
          } else throw Response.createError({ message: 'signatures dont match' });
    } catch (err) {
